...
import { item, chapters } from '$lib/server/library/queries';
...
export const load = async ({ locals, params }) => {
	...
	const [chapterRows, bookmarkRows] = await Promise.all([
		chapters(db, target.id),
		bookmarksForItem(db, userId, target.id)
	]);
	return { item: target, chapters: chapterRows, bookmarks: bookmarkRows };
};
